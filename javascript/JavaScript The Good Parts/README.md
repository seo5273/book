...
함수 호출 패턴으로 호출 할때 `this`는 전역객체에 바인딩된다.
...
> 이런 문제에 대한 대안은 메소드에서 변수를 정의한 후 여기에 this를 할당하고, 내부 함수는 이 변수를 통해서 메소드의 this에 접근하는 방법이 있다<br>.
...
> replacement 가 함수이면 첫번째 인자는 패턴에 매치된 문자열, 두번째 인자는 패턴 표현식에 매치된 문자열들이다.<br>

모듈의 일반적인 패턴은 private 변수와 함수를 정의하는 함수다.<br>
클로저를 통해 private 변수와 함수에 접근할 수 있는 권이 있는 함수를 생성하고<br>
이 함수를 반환하거나 접근 가능한 장소에 이을 저장하는 것이다. 

모둘 패턴은 안전한 객체를 생성하는데도 사용할 수 있다.
...
메소드가 this 나 앞서 살펴본 that (03절의 함수 호출 패턴)을 사용하지 않기 때문에 seqer 내부의 변수를 접근할 수 있는 방법은 없다.<br>
즉 해당 변수를 다루도록 정의된 메소드를 제외하고는 prefix 나 seq의 값을 얻거나 변경할 수 있는 방법은 없다.<br>
seqer 객체의 메소드들은 다른 메소드로 대체 될 수 있지만 대체 된 메소드들이 숨겨져 있는 prefix 나 seq 를 접근할 수는 없다.

#### 13 | 연속 호출(Cascade)
****
일부 메소드는 반환값이 없다. 예를 들어 객체의 상태를 변경하거나 설정하는 메소드들은 일반적으로 반환값이 없다.<br>
만약 이러한 메소드들이 undefined 대신에 this를 반환한다면 연속호출이 가능하다.

	// getElement 예제
	// 각각의 메소드는 객체를 반환하기 때문에 각 메소드 호출 결과를 다음 호출에 사용 할 수 있다.
	getElement('myBoxDiv')
		.move(350, 150)
		.width(100)
		.height(100)
	...
	app.use = function(route, fn){
		...
		return this;						// this 를 return 한다.
	};


#### 14 | 커링 (Curry)
****
함수는 값(value)이며, 이 함수값을 흥미로운 방법으로 다룰 수 있다.<br>
[커링](http://en.wikipedia.org/wiki/Currying)은 함수와 인수를 결합하여 새로운 함수를 만들 수 있게 한다.

아래 예제 중 <br>
add1은 add의 curry 메소드에 1을 넘겨서 생성한 함수다. add1 함수는 자신의 인수에 1을 더한다.
	...
	// 위 예제 중...
	// arguments 가 배열 같지만 배열이 아니라서 배열의 메소드가 없다.
	// 때문에 array.slice 로 배열을 return 받아 concat 기능을 사용한다. 
	Function.method('curry', function () {
	    var slice = Array.prototype.slice,
            args = slice.apply(arguments),					// slice 는 새로운 배열을 반환 한다.
			that = this;									// 함수 호출 패턴의 문제점 예제 참고할 것.
...
****
함수는 불필요한 작업을 피하기 위해서 이전에 연산한 결과를 저장하고 있는 객체를 사용할 수 있다.<br>
...
### 7. 정규 표현식
====
정규 표현식은 간단한 언어 구문의 특화된 형태이다. 정규 표현식은 문자열에서 특정 내용을 찾거나 대체 또는 발췌하는데 사용한다.<br>
정규 표현식을 사용하는 메소드는 `regexp.exec`, `regexp.test`, `string.match`, `string.replace`, `string.search`, `string.split` 등이 있다.<br>
자바스크립트에서 정규 표현식은 보통 같은 문자열에 대해 반복해서 연산을 수행할 때 주목할 만한 성능상의 이점이 있다.

#### 01 | 예제
****
자바스크립트 프로그램에서 정규 표현식은 한 줄이어야하고 공백문자(whitespace)는 사용할 수 없다.
...
`^` 문자는 문자열의 시작을 나타낸다. 이 문자는 exec 메소드가 URL 같지 않은 시작을 보이는 문자열은 바로 건너뛰게 하는 역할을 한다.
